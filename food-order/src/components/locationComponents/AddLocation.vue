...
const props = defineProps({
    visible: Boolean,
    title: String,
    data: { type: Object, default: () => ({ address: "", isDefault: false }) },
})
...
// Reset form và khởi tạo map khi mở popup
watch(
    () => props.visible,
    async (val) => {
        if (val) {
            ...
            // ❌ Nếu có map cũ thì remove trước
            if (map) {
                map.remove()
                map = null
                marker = null
            ...
            // ✅ Tạo lại map mới mỗi lần mở popup
            map = new window.goongjs.Map({
                container: "map",
                style: "https://tiles.goong.io/assets/goong_map_web.json",
                center: [105.83416, 21.027764], // Hà Nội
                zoom: 12,
            })
            ...
            // Nếu có sẵn lat/lng trong form (trường hợp sửa địa chỉ) thì load lên map
