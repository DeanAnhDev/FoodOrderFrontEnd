<template>
  <section v-bind="$attrs" class="mb-10">
    ...
    <p v-if="foodCategoriesWithFoodsStore.error" class="text-center text-red-500">
      {{ foodCategoriesWithFoodsStore.error }}
    </p>

    <div v-else-if="categoriesWithFoods.length > 0" v-for="category in categoriesWithFoods" :key="category.id"
      class="mt-5">
      ...
      <FoodComponent :category="category" />
    </div>
